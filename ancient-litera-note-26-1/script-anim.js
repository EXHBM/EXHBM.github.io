/* ═══════════════════════════════════════════════════════════════
   笔记页模板 · 组件库第 2 部分 + 滚动动画（原样取自 jyy-lec2-note，tag 0.1.1）
   所有组件均带容器守卫：HTML 里没有对应 id 就自动跳过。
   ── 组件库（容器标记参考见模板 index.html 末尾注释块）──
   · 滑杆探针（track/miao 阈值报警）   → 容器 #x4track #x4fill #x4mks #x4sl #x4miao #x4st，数据在 MK 数组
   · 靶心对比（target 双按钮射箭）     → 容器 #x7arr #x7sc #x7a #x7b #x7st
   · 循环计数（loop 四步 + 轮数统计）  → 容器 #x8loop #x8n #x8f #x8st
   · 纸带 FSM（三幕：状态机→纸带→回读）→ 容器 #x5fsm #x5tape #x5act #x5st
   · 尝试列表（逐步展示成功/失败）     → 容器 #x6list #x6st，数据在 T6 数组
   ── 滚动触发动画 ──
   · .anim-fig 容器内的 SVG 滚进视口加 .go 播放、离开重置
   · 原语：.grow/.ln1/.ln2 描边生长、.flow 流动虚线、.fadein(.d2/.d3/.d4) 淡入、.wall 竖起、.pulse 脉冲
   ═══════════════════════════════════════════════════════════════ */
